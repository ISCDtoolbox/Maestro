program cc_ff_autocorr
  ...
  integer nstep_therm,cc,len_file,nstep_after_therm
  integer i1,i2,i3,i4,i5,lworkd,skipGF,iendGV,lbin,nbin
  integer cc1,cc2
  ...
  write(*,*) 'I) bin lenght, II) iendGV (if <= 0 until the end), III) skipGF'
  read(5,*) nbin, iendGV, skipGF

  nstep_therm=nstep_block/iprint
  nstep_after_therm=nstep_block*(nblocks)/iprint-nstep_therm
  if (irun.eq.3) then
    dt=iprint*delt*timeau2fs
  else
    dt=iprint*delt*timeau2fs*sqrt(amas(1))
  end if

  write(*,*) '------------------------------------------------------------------------------------------'
  write(*,*) 'Calculations of phonons with covariance force matrix within GF and <vv> correlation method'
  write(*,*) 'This program works for an isolated system - molecule'
  write(*,*) '------------------------------------------------------------------------------------------'
  write(*,*)
  write(*,*) '# of thermalizion steps: ', nstep_therm
  write(*,*) '# of "potential" dynamical steps: ', nstep_after_therm
  write(*,*) 'Temperature in Kelvin: ', tempMD
  write(*,*) '# of beads: ', nbeadMD
  write(*,*) 'dt*iprint of the MD simulation [fs]: ',dt
  write(*,*) '# of degrees of freedom', ncolvars
  write(*,*)



  OPEN(UNIT=12,FILE='configinit_zu.xyz',STATUS='OLD',iostat=iflagerr)
...
!!! compute the length of vectors which depends on how many configurations you want to skip
  ...
  rewind(16)
  write(*,*) 'test over file lenght: ',len_file
...
!!! ...and read files and skip some configurations to have decorrelated configurations (this must be tested with autocorr.f90!)
  cc=0
  do i1=1,nstep_after_therm,skipGF
    cc=cc+1
